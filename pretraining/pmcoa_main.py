...
from model_vl_t5 import T5ForConditionalGeneration, T5ForMultimodalGeneration
# from vqa_model import T5ForVQAMultimodalGeneration

from pmcoa_utils_data import load_pmcoa_data_img, PMCOADatasetImg, img_shape
...
def parse_args():
    ...
    parser.add_argument('--prompt_format', type=str, default='QCM-A', help='prompt format template',
                        choices=['QCM-A', 'QCM-LE', 'QCMG-A', 'QCM-LEA', 'QCM-ALE', 'QCG-A', 'QC-EA', 'QC-E', 'QC-LE','Q-A'])
...
def T5Trainer(
    ...
    if args.img_type is not None:  # img_type不为空
        ...
        # load model
        model = T5ForMultimodalGeneration.from_pretrained(args.model, patch_size=patch_size)  # T5为多模态训练设置的模型
        model = model.to("cuda")
    ...
    if trainer.is_world_process_zero():
        ...
        output_data = {
            ...
            "preds": preds,
            "labels": targets}
        output_prediction_file = os.path.join(save_dir, "pmcoa_predictions_ans_test.json")
    ...
    if args.prompt_format == "QCM-LE":
        ...
        if trainer.is_world_process_zero():
            ...
            output_data = {"preds": preds,
                           "labels": targets}
            output_prediction_file = os.path.join(save_dir, "pmcoa_predictions_ans_eval.json")
            with open(output_prediction_file, "w") as writer:
                writer.write(json.dumps(output_data, indent=4))
...
if __name__ == '__main__':
    ...
    if args.img_type is not None:
        problems, qids, name_maps, train_image_features, val_image_features, test_image_features = load_pmcoa_data_img(
